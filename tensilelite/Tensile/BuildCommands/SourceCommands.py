...
def _buildSourceCodeObjectFile(cxxCompiler: str, offloadBundler: str, outputPath: Union[Path, str], fromTensile: bool, kernelPath: Union[Path, str]) -> List[str]:
...
def buildSourceCodeObjectFiles(cxxCompiler: str, offloadBundler: str, kernelFiles: List[Path], outputPath: Path, fromTensile: bool) -> Iterable[str]:
    ...
    Returns:
        List of paths to the created code objects.
    """
    args    = zip(itertools.repeat(cxxCompiler), itertools.repeat(offloadBundler), itertools.repeat(outputPath), itertools.repeat(fromTensile), kernelFiles)
